...
| Setting                            | Default                                | Description                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MaxSystemsPerTargetGrid`          | `20` (single-player) / `10` (multiplayer) | Maximum number of Build and Repair blocks that may work on the same target grid simultaneously. Prevents many systems piling onto one grid while nearby grids are left untouched. The default is **20** in single-player games and **10** in any multiplayer session (hosted or dedicated). Setting this in the config file overrides whichever default applies. Accepted range: 1–100.                                                                |
...
| Setting                       | Default    | Description                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `StaggerGroupCount`           | `0` (auto) | Controls how many stagger groups are used to distribute Build and Repair updates across ticks. `0` means automatic — the mod picks the number based on how many **enabled** Build and Repair blocks are currently in the world (≤5 = 1 group, ≤10 = 2, ≥11 = 3). Disabled blocks are ignored, so toggling unused systems off restores responsiveness for the active fleet. Higher values spread the load more but increase the delay between each block's updates. Accepted range: 0–10. |
...
| `MaxWeldsPerTick`             | `0` (auto) | Global cap on weld operations across all blocks per tick. `0` means automatic — the mod scales the budget based on block count (minimum 5, maximum 10). Setting a fixed value overrides the auto scaling. Higher values allow faster welding but risk frame spikes when the underlying engine weld call is heavy on a tick. Accepted range: 0–100.                                                |
| `MaxGrindMsPerTick`           | `8`        | Per-tick wall-clock budget (in milliseconds) for grind work across all blocks. Caps total grinding time per tick so engine spikes can't push a frame past the sim budget. With many systems on a server the ms cap often wins over `MaxGrindsPerTick` — raise this to let more systems grind in parallel each tick. Recommended starting point for fleets of 30+ systems: `30`. Accepted range: 1–100. |
| `MaxWeldMsPerTick`            | `8`        | Per-tick wall-clock budget (in milliseconds) for weld work across all blocks. Same role as `MaxGrindMsPerTick` for the weld path. Engine weld calls can spike 5–30 ms on complex blocks (cockpits, hydrogen tanks, etc.), so raising this can let more systems weld in parallel at the cost of occasional larger tick spikes. Accepted range: 1–100.                            |
| `BlockFailureCooldownSeconds` | `15`       | After a block fails to weld because its missing components are unavailable from every source (or a projector exception, etc.) it is placed on a global fail-cooldown for this many seconds. Other Build and Repair blocks — and the same block on later ticks — skip the cooldowned block, so a large set of high-priority blocks all needing one exhausted component can't keep the system bouncing off them every tick instead of welding lower-priority blocks that do have components. Set to `0` to disable. Accepted range: 0–30.          |
...
| Setting                        | Default | Description                                                                                                                                                                                                                                         |
| ------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LogLevel`                     | `Error` | Controls how much detail is written to the mod log. Accepted values: `Error`, `Event`, `Info`, `Verbose`, `Communication`, `All`. Use anything above `Error` only when troubleshooting — it generates a large amount of output.                     |
