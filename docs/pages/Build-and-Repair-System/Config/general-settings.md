...
| `MaxInventoryFullPushAttempts`     | `100`                                  | Number of consecutive 5-second push cycles with a full inventory and no active welding before the system automatically disables itself. Accepted range: 1–500. Prevents a block with a backed-up conveyor network from continuously attempting futile push operations.                                                                                                                                                                |
| `EmptyGridRescanDelaySeconds`      | `30`                                   | After scanning a grid and finding no weld or grind targets, the system will skip that grid for this many seconds before scanning it again. This reduces CPU load from repeatedly iterating grids that have nothing for the system to do. Set to `0` to disable this behaviour. Sub-grid connections (connectors, pistons, rotors) are always traversed regardless of this setting, so a newly docked or spawned ship is never missed. |
...
| `EnableMethodProfiling`        | `false` | Enables method-level profiling logs written to local storage. The mod writes one file per instrumented method (`NanobotProfiler.<MethodName>.log`) with elapsed time and key variables. Keep disabled unless actively profiling performance issues. |
| `MethodProfilingMinDurationMs` | `1`     | Minimum method runtime in milliseconds required before a profiling sample is written. Increase this value to reduce log volume and focus on slower calls. Range: `0`-`10000`.                                                                       |
