...
| FEAT-075 | High | v2.5.4 | In Progress | Saturated scan skip — when coordinator still has >64 live targets (25% of 256 cap), skip the expensive full-grid rescan entirely. Quick-count live targets (~12µs) instead of running the 50ms+ scan. Members also skip. Safety: force rescan every 60s. On 10K-block grind, expected to reduce scans from ~17/180s to ~3-5/180s |
