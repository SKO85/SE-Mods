...
| BUG-101 | High | v2.5.3 | `NanobotSystem.Welding.cs`, `NanobotSystem.Operations.cs` | TODO | Weld-loop exhaustion fast-path (`_weldLoopExhausted`) can misclassify grid-limited systems as "idle/no work" when all candidates are skipped by `MaxSystemsPerTargetGrid`, then keep skipping re-attempts while target hash is unchanged. This can under-utilize available slots and match issue #133 symptoms (only one/few active until limiter is disabled). |
