...
| `AssignToSystemEnabled` | `true` | When `true`, the Assign to System feature is available. This allows blocks to be assigned to a specific Build and Repair system. |
...
| Setting | Default | Description |
| --- | --- | --- |
| `MaxSystemsPerTargetGrid` | `10` | Maximum number of Build and Repair blocks that may work on the same target grid simultaneously. Prevents many systems piling onto one grid while nearby grids are left untouched. |
